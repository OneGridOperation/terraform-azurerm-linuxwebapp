...
resource "azurerm_linux_web_app" "linux_web_app" {
  ...
  site_config {
    ...
    #dynamic "ip_restriction" {
    #  for_each = try(var.configuration.site_config.ip_restrictions, null) != null ? var.configuration.site_config.ip_restrictions : []
    #  content {
    #    name        = ip_restriction.value.name
    #    ip_address  = contains(keys(ip_restriction.value), "ip_address") ? ip_restriction.value["ip_address"] : null
    #    service_tag = contains(keys(ip_restriction.value), "service_tag") ? ip_restriction.value["service_tag"] : null
    #    action      = contains(keys(ip_restriction.value), "action") ? ip_restriction.value["action"] : null
